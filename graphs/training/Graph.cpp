...
{
    auto topoSortedList = getTopologicalOrder();

    return topoSortedList.size() != m_nodeCount;
}
...
bool Graph::isCyclicRecursiveForDirectedGraph()
{
    ...
    {
        if(!visited[i])
            if(isCyclicRecursiveHelperForDirectedGraph(i, visited, inStack))
                return true;
    }
...
vector<int> Graph::getTopologicalOrder()
{
    ...
    for(int i = 0 ; i < m_nodeCount ; i++)
        for(int neighbor : m_adjacencyList[i])
            inDegrees[neighbor]++;
...
bool Graph::isCyclicRecursiveHelperForDirectedGraph(int node, vector<bool>& visited, vector<bool>& inStack)
{
    if(!visited[node])
    {
        visited[node] = true;
        ...
        for(const auto neighbor : m_adjacencyList[node])
        {
            if(visited[neighbor] && isCyclicRecursiveHelperForDirectedGraph(neighbor, visited, inStack))
                return true;
...
void Graph::dfsKosaraju2(int node, const vector<vector<int>>& reverseAdjancencyLists, vector<bool>& visited, vector<int>& scc)
{
    ...
    for(const auto neighbor : reverseAdjancencyLists[node])
        if(!visited[neighbor])
            dfsKosaraju2(neighbor, reverseAdjancencyLists, visited, scc);
}
